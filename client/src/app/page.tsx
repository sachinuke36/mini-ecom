    ...
    <div className="flex justify-evenly items-center px-20">
      <div className="shadow-lg rounded-2xl px-2 bg-white  shadow-purple-500 flex items-center justify-center gap-2">
        <input type="search" 
            ...
            name="" id="" />
            <FaSearch size={20}  color="purple" className="font-extrabold mx-auto" />
      </div>
      <Navbar active={active} setActive={setActive}/>
      <div className="shadow-lg p-2 rounded-2xl shadow-purple-500 bg-white">
        <p className="font-bold text-amber-800 text-shadow-sm">{user?.email || "User name"}</p>
      </div>
      ...
      <div>
        {
          active=='add-product' ? <AddProduct/> : <MyProducts data={filteredData}/>
        }
      </div>
    </div>
    </div>
